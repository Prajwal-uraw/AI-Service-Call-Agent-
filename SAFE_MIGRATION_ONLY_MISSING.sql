...
-- 1. Uses IF NOT EXISTS - won't touch existing tables
-- 2. Won't modify any existing data
-- 3. Won't break Twilio numbers or AlertStream
-- 4. Only adds what's missing
-- =====================================================
...
-- =====================================================
-- TENANTS TABLE (Base table - required for everything)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS tenants (
    ...
    -- Twilio Configuration (PRESERVES EXISTING SETUP)
...
-- =====================================================
-- CALL LOGS TABLE (PRESERVES EXISTING TWILIO DATA)
-- =====================================================
...
-- =====================================================
-- TRIGGERS (Safe - will only create if missing)
-- =====================================================
...
-- Apply triggers (DROP IF EXISTS makes this safe)
...
-- Auto-create AI config for new tenants
...
BEGIN
    ...
    ON CONFLICT (tenant_id) DO NOTHING;  -- Safe: won't overwrite existing configs
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- =====================================================
-- INSERT DEFAULT TEMPLATES (Safe - ON CONFLICT DO NOTHING)
-- =====================================================
...
BEGIN
    ...
    RAISE NOTICE '✅ Existing data was NOT modified';
    RAISE NOTICE '✅ Twilio configuration preserved';
    RAISE NOTICE '✅ AlertStream data preserved';
...
SELECT 
    tablename,
    CASE 
        WHEN tablename IN ('tenants', 'tenant_users', 'call_logs', 'appointments', 'ai_agent_configs')
        THEN '✅ CRITICAL TABLE'
        ELSE '✅ SUPPORT TABLE'
    END as importance
FROM pg_tables 
...
AND tablename IN (
    'tenants', 'tenant_users', 'tenant_api_keys', 'call_logs',
    'appointments', 'usage_tracking', 'ai_agent_configs',
    'ai_config_templates', 'ai_config_audit_log'
)
ORDER BY tablename;
